...
static const qint32 BroadcastInterval = 2000;  // 2 sec interval
static const unsigned broadcastPort = 8516;    // cmd port + 1

#include "peermgr.hpp"


PeerManager::PeerManager(TCPClient *tcpclient) : QObject(tcpclient)
{
    this->client = tcpclient;

    connect(this, SIGNAL(newConnection(TCPConnection *)),this, SLOT(slotNewConnection(TCPConnection *)));
...
bool PeerManager::isLocalHostAddress(const QHostAddress &address)
{
    ...
    {
        if (address == localAddress)  return true;
    }
    return false;
}

void PeerManager::slotNewConnection(TCPConnection *connection)
{
    Q_UNUSED(connection)
}
...
void PeerManager::updateAddresses()
{
    ...
    foreach (QNetworkInterface interface,QNetworkInterface::allInterfaces())
    {
        ...
        foreach (QString mac,interface.hardwareAddress())
        {
            macaddresses << mac;
        }
    }
