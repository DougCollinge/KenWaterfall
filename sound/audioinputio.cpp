...
AudioInputIO::AudioInputIO(QObject *parent) : QIODevice(parent)
{
#ifdef DEBUG_AUDIO
...
AudioInputIO::~AudioInputIO() { }

void AudioInputIO::setAudioFormat(QAudioFormat format)
{
#ifdef DEBUG_AUDIO
...
#endif
    emit signalAudioInputOK();
}

void AudioInputIO::stop()  { close(); }

qint64 AudioInputIO::readData(char *data, qint64 maxlen)
{
    // not used here
    Q_UNUSED(data)
    Q_UNUSED(maxlen)
...
qint64 AudioInputIO::writeData(const char *data, qint64 len)
{
    ...
    m_level = real(value) / double(m_maxAmplitude);
