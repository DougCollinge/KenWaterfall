...
void TNCPanel::initializeFromIni()
{
       ...
       )
    {
        ...
        mwd->strCaptureDev    = devCaptureDevice.deviceName();
    }
    mwd->intRxVolume = mcb->RxVolume;
    ui->ctrlVol->setValue(mwd->intRxVolume);
...
void TNCPanel::initializeAudio()
{
    ...
    else
    {
        ...
        qInfo() << "No - so find the device "
                << mwd->strCaptureDev << "in the available devices";
#endif
        m_format.setSampleType(QAudioFormat::SignedInt);  // default for now
        m_format.setByteOrder(QAudioFormat::Endian(QSysInfo::ByteOrder));
        m_format.setChannelCount(mwd->intChannels);
        m_format.setSampleSize(16);                       // default for now
        m_format.setSampleRate(mwd->intRxsmplrate);
        m_format.setCodec("audio/pcm");                   // Linear PCM codem - our only audio codec for now
#ifdef DEBUG_AUDIO
...
#endif
        int x = 0;
        int y = mwd->cllCaptureDevices.count();
        for ( int x = 0; x < y; ++x )
        {
            if ( mwd->cllCaptureDevices.at(x).deviceName() == mwd->strCaptureDev ) break;
        }
        devCaptureDevice = mwd->cllCaptureDevices.at(x);
        mwd->strCaptureDev = devCaptureDevice.deviceName();
        mcb->CaptureDevice = mwd->strCaptureDev;
#ifdef DEBUG_AUDIO
        qDebug() << "Dev:" << devCaptureDevice.deviceName() << "selected from settings";
#endif
    }
#ifdef DEBUG_AUDIO
    ...
    connect(audioIOObject, SIGNAL(signalAudioInputOK()),this, SLOT(slotAudioInputOK()));
...
void TNCPanel::createAudioInput()
{
    ...
    audioInput->setVolume(mwd->intRxVolume);
    audioIOObject->start();
...
void TNCPanel::createLocalObjects()
{
    ...
    mwd->tmrPoll = new QTimer(this);
    mwd->tmrStartCODEC = new QTimer(this);
...
void TNCPanel::connectAndSetupUi()
{
    // connect modem signals to GUI for starting setup
//    connect(objMain,SIGNAL(signalCtrlChgd(QString,QColor,QString,quint32)),this, SLOT(slotCtrlChgd(QString, QColor, QString, quint32)));
}

void TNCPanel::audioInputError(QAudio::Error e)
{
...
void TNCPanel::readSettings()
{
    ...
    settings.beginGroup("SOUND");
     ...
     mwd->intChannels    = settings.value("RxChannels",          1).toInt();
    settings.endGroup();
    // update local vars with control block values
    mwd->strCaptureDev   = mcb->CaptureDevice;
    mwd->strPlayDev      = mcb->PlaybackDevice;
    mwd->intRxsmplrate   = mcb->RxAudioSampleRate;
...
{
//    QDir dir; // dir.canonicalPath() + "/ardop_tnc.ini"
    QSettings settings("/ardop_tnc.ini",QSettings::IniFormat);
